...
**💡 Tip:** Use `print` function for debugging as it's more stable than `log`.
...
**💡 Recommendation:** `setState`/`getState` supports more data types and is preferred.
...
**⚠️ Critical Bug:** The `setDescription` function is currently broken due to a variable reference error in the source code (references `data` instead of `desc` parameter).
...
```lua
-- ❌ Inefficient: Multiple full chat history scans
...
```lua
-- ❌ Inefficient: Unnecessary async operations in loop
function onOutput(triggerId)
    for i = 1, 10 do
...
-- ✅ Efficient: Cache and conditional execution
function onOutput(triggerId)
...
```lua
-- ❌ Inefficient: Recalculating every time
function onOutput(triggerId)
    local stats = {}
    for i = 0, getChatLength(triggerId) - 1 do
        local message = getChat(triggerId, i)
